...
def parse_alert(text, repos=None):
    ...
    if not known:
        result["notes"].append(
            ...
            "cannot identify the affected repo. Run make_repo_tags.py on the box.")
        return result
    ...
    if not result["repos"]:
        ...
        if not hinted:
            result["notes"].append(
                "no known repo id appears in this alert text, and no resource_repo_hints entry "
                "matched. NOT guessing. To fix permanently, add the resource->repo mapping to "
                "config/alarm_patterns.json (intranet-owned).")
    result["identified"] = bool(result["repos"])
...
def incident_impact(alert_text, repos=None, max_use_cases=_MAX_USE_CASES_LISTED):
    ...
    if not parsed["identified"]:
        result["ok"] = False
        result["error"] = "could not identify an affected repo from this alert text"
        result["next_step"] = (
            "Tell the user which part of the alert you could and could not read, and ask for the "
            "service/repo name. Do not guess.")
        return result
    ...
    result["totals"] = {
        ...
        "use_cases": sum(item.get("use_case_total", 0) for item in result["affected"]),
        "channels": sorted({c for item in result["affected"] for c in item.get("channels") or []}),
    }
    return result
