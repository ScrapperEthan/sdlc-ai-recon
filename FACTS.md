...
| **"全链路"现在到出口了**(主题→投递任务→出站 API→厂商→SMSC/APNs/ProofPoint/打印),`usecase_impact.delivery_chain` + 架构图高亮都通到底。但**厂商默认是渠道级上界**(`channel_upper_bound`),因为 `tbl_use_case_router` 还没摄取 —— 只能说"最多这几家",**不能说"它走 X"**。`route`/`router` 里出现厂商名时收窄为 `route_hint`,仍是线索(RB-54 问题 1 未答) | 2026-07-29 外部 |
